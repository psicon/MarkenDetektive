 ...
 var cartitems = [    			{ 	itemname: $scope.data.bezeichnung, 
  ...
  $scope.deletecart = function() {
	...
	if(suche==""){$ionicPopup.alert({
		...
		SuchService.setIDorEAN("http://product-api.affili.net/V3/productservice.svc/json/SearchProducts?publisherid=708669&password=xg5I8J6vCNRNpY2y6pkm&PageSize=4&ImageScales=Image180&LogoScales=Logo120" + ProdukteService.getShops() + "&query=" + suche);
		var today = new Date();
  ...
  $scope.scanbarcode = function(){
    ...
    window.plugins.flashlight.switchOff(); // success/error callbacks may be passed
				...
				scanned= "0" + result.text;
 				...
 				SuchService.setIDorEAN("http://product-api.affili.net/V3/productservice.svc/json/SearchProducts?publisherid=708669&password=xg5I8J6vCNRNpY2y6pkm&PageSize=4&ImageScales=Image180&LogoScales=Logo120" + ProdukteService.getShops() + "&query=EAN:(" + scanned + " OR " + result.text+" OR " + scanned2 +")");
var today = new Date();
var dd = today.getDate();
var mm = today.getMonth()+1; //January is 0!
var yyyy = today.getFullYear();
...
.controller('LoadingCtrl', function($scope, $ionicLoading) {
 ...
 $scope.opendetails = function (productid, productean){if(productean == ""){
					SuchService.setIDorEAN("http://product-api.affili.net/V3/productservice.svc/json/GetProducts?publisherid=708669&password=xg5I8J6vCNRNpY2y6pkm&PageSize=4&ImageScales=Image180&LogoScales=Logo120" + ProdukteService.getShops() + "&ProductIds=" +productid);				}else{
				...
				scanned= "0" + productean;
 				...
 				ProdukteService.setProdukt("EAN:(" + scanned + " OR " + productean+" OR " + scanned2 +")");
					//alert("EAN:" + scanned + " EAN:" + result.text);
SuchService.setIDorEAN("http://product-api.affili.net/V3/productservice.svc/json/SearchProducts?publisherid=708669&password=xg5I8J6vCNRNpY2y6pkm&PageSize=4&ImageScales=Image180&LogoScales=Logo120" + ProdukteService.getShops() + "&query=EAN:(" + scanned + " OR " + productean +" OR " + scanned2+")");
				}
				$state.go("app.preisvergleich");}
   ...
   var hideSheet = $ionicActionSheet.show({
     ...
     buttonClicked: function(index) {
      ...
      destructiveButtonClicked: function() {
				if(productean == ""){
					 SuchService.setIDorEAN("http://product-api.affili.net/V3/productservice.svc/json/GetProducts?publisherid=708669&password=xg5I8J6vCNRNpY2y6pkm&PageSize=4&ImageScales=Image180&LogoScales=Logo120" + ProdukteService.getShops() + "&ProductIds=" +productid);				
					}else{
...
console.log("Suchlink: http://psicon.de/discountermarken/ionic/php/getprodukte_bysearch.php?searchitem=" + SuchService.getSuchbegriffdetektive());
 ...
 .controller('getProduktebysearchexternCtrl', function($scope,$ionicActionSheet, $location, $window, $http, $ionicLoading, $state,$ionicPopup, ShoppingCartService,$ionicModal, ProdukteService, SuchService) {
   							   	$ionicLoading.show();
...
var indicator = (data.ProductsSummary.TotalRecords/data.ProductsSummary.TotalRecords);
    ...
    $scope.$broadcast('scroll.infiniteScrollComplete');
										$scope.$apply();
																																		 ...
																																		 $scope.moreavailable = true; 
 //$scope.produkte.push.apply($scope.produkte,data.Products);
																			 angular.forEach(data.Products,function(item) {
																				 //alert('try:' +item.ProductName);
